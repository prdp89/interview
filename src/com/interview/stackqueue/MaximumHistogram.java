...
public class MaximumHistogram {
    ...
    public int maxHistogram( int input[] ) {
        ...
        for (i = 0; i < input.length; ) {
            if (stack.isEmpty() || input[stack.peekFirst()] <= input[i]) { //insert when next input element is greater
                stack.offerFirst(i++); //insert ith item into the stack
            } else {
                int top = stack.pollFirst(); //remove top element from stack
