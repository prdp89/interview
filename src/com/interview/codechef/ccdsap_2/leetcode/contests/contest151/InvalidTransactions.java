package com.interview.codechef.ccdsap_2.leetcode.contests.contest151;

import java.util.ArrayList;
import java.util.List;

public class InvalidTransactions {

    //https://leetcode.com/contest/weekly-contest-151/problems/invalid-transactions/
    public static void main( String[] args ) {
        String[] list = {"alice,20,800,mtv", "alice,50,100,beijing"};

        invalidTransactions(list).forEach(System.out::println);
    }


    private static List<String> invalidTransactions( String[] transactions ) {

        List<String> ans = new ArrayList();
        int n = transactions.length;

        if (n == 0)
            return ans;

        String[] name = new String[n];
        int[] time = new int[n];
        int[] money = new int[n];
        String[] city = new String[n];
        boolean[] add = new boolean[n];

        for (int i = 0; i < n; i++) {
            String[] items = transactions[i].split(",");
            name[i] = items[0];
            time[i] = Integer.parseInt(items[1]);
            money[i] = Integer.parseInt(items[2]);
            city[i] = items[3];
        }

        for (int i = 0; i < n; i++) {
            if (money[i] > 1000)
                add[i] = true;

            for (int j = i + 1; j < n; j++) {
                if (name[i].equals(name[j])
                        && Math.abs(time[i] - time[j]) <= 60
                        && !city[i].equals(city[j])) {
                    add[i] = true;
                    add[j] = true;
                }
            }
        }

        for (int i = 0; i < n; i++) {
            if (add[i])
                ans.add(transactions[i]);
        }

        return ans;
    }

}
